...
tion.nix and flake method we're all used to in the NixOS ecosystem.
This tutorial guides you through installing NixOS Mobile on a OnePlus 6 (enchilada) device. The process involves building a minimal base image using the local.nix file (with fixes to make it build on x86 machines), flashing it to your device, then rebuilding on the phone itself (aarch64) with the full configura
* Clone Your Configuration Repository

First, clone or download this nixos-mobile repository to get you started with a minimal config to build:

#+begin_src sh
git clone https://github.com/mwlaboratories/nixos-mobile.git
cd nixos-mobile
#+end_src

* Build Flashable Images

We will import the official mobile-nixos repo to build from. It will be a folder `mobile-nixos` under our nixos-mobile project directory. After building, we can delete it as it is only needed to build our base image for one-time use:
